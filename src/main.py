...
def save_matches(pairs: list):
    """Persist digest jobs so the draft CLI can generate cover letters."""
    path = os.path.join(ROOT, "data", "matches.json")
    try:
        with open(path, encoding="utf-8") as f:
            matches = json.load(f)
    ...
    for job, r in pairs:
        matches[job.url] = {"title": job.title, "company": job.company,
                            "score": r["match_score"], "date": str(date.today()),
                            "description": job.description[:4000]}
    matches = dict(sorted(matches.items(), key=lambda x: x[1]["date"])[-200:])
...
def run(dry_run: bool = False):
    ...
    # 4b. Subagents: company intel for top matches; save digest jobs for the
    # cover-letter draft CLI (python -m src.agents.draft --list).
    if top:
        from .agents.intel import briefing
        for job, r in top:
            r["intel"] = briefing(job.company, job.description)
